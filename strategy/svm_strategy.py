...
class SVMStrategy(Strategy):
    ...
    def place_trade(self, curr_idx: int, market_data: DataFrame) -> Optional[Trade]:
        ...
        svm_buy_signal = argmax == TradeType.BUY.value and proba >= self.proba_threshold
        svm_sell_signal = argmax == TradeType.SELL.value and proba >= self.proba_threshold

        trade = None

        if svm_buy_signal or svm_sell_signal:
    ...
    def run_strategy(self, currency_pair: CurrencyPairs, aat_trainer: Optional[AatMarketTrainer] = None,
        ...
        self.model = pickle.load(open(f'../nn/training_data/{self.currency_pair.value}_trained_svm.pickle', 'rb'))
        self.scaler = pickle.load(open(f'../nn/training_data/{self.currency_pair.value}_trained_svm_scaler.pickle',
                                       'rb'))
