...
class EMACrossStrategy(Strategy):
    def __init__(self, starting_idx: int, risk_reward_ratio: float, spread_cutoff: float, lookback: int):
        description = f'EMA Crossover strategy with {risk_reward_ratio} risk/reward, {spread_cutoff} spread ratio, ' \
                      f'{lookback} lookback'
        Strategy.__init__(self, 'ema_strategy', description, starting_idx)
    ...
    def place_trade(self, curr_idx: int, market_data: DataFrame) -> Optional[Trade]:
        ema200_2, ema100_2 = market_data.loc[market_data.index[curr_idx - 2], ['ema200', 'ema100']]
        ...
        buy_signal = ema100_2 < ema200_2 and ema100_1 > ema200_1
        sell_signal = ema100_2 > ema200_2 and ema100_1 < ema200_1
